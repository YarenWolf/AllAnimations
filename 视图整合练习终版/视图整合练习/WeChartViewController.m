...
@end
@interface WeChartViewController ()<UITableViewDataSource,UITableViewDelegate>
...
-(NSMutableArray *)allMessage{
    if(!_allMessage){
        _allMessage = [[Data demoData]mutableCopy];
    }return _allMessage;
}
...
- (IBAction)clickReturnKey:(UITextField *)sender {//点击右下角返回键，发消息，手键盘。
    ...
    Data *newMessage=[[Data alloc]init];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Data *mssg=self.allMessage[indexPath.row];
    //显示内容
